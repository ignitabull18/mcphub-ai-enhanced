...
const ToolCard = ({ tool, server, onToggle, onDescriptionUpdate }: ToolCardProps) => {
  ...
  return (
    <div className="bg-white border border-gray-200 shadow rounded-lg p-3 sm:p-4 mb-3 sm:mb-4">
      <div
      ...
      >
        <div className="flex-1">
          <h3 className="text-base sm:text-lg font-medium text-gray-900">
            ...
            <span className="ml-2 text-xs sm:text-sm font-normal text-gray-600 inline-flex items-center">
              {isEditingDescription ? (
                <>
                  <input
                    ref={descriptionInputRef}
                    type="text"
                    className="px-2 py-1 border border-blue-300 rounded bg-white text-xs sm:text-sm focus:outline-none form-input"
        ...
        <div className="flex items-center justify-between sm:justify-end space-x-2">
          ...
          <button
            ...
            className="flex items-center space-x-1 px-2 sm:px-3 py-1 text-xs sm:text-sm text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-md transition-colors btn-primary"
      ...
      {isExpanded && (
        <div className="mt-4 space-y-4">
          {/* Schema Display */}
          {!showRunForm && (
            <div className="bg-gray-50 rounded p-2 sm:p-3 border border-gray-300">
              <h4 className="text-xs sm:text-sm font-medium text-gray-900 mb-2">{t('tool.inputSchema')}</h4>
              <pre className="text-[10px] sm:text-xs text-gray-600 overflow-auto">
