...
const virtualServers: Map<string, VirtualMcpServer> = new Map();
// Cache for virtual server tools
const virtualServerToolsCache: Map<string, ToolInfo[]> = new Map();
...
export const getServersInfo = async (): Promise<Omit<ServerInfo, 'client' | 'transport'>[]> => {
  ...
  // Add virtual servers as server infos
  for (const [name, virtualServer] of virtualServers) {
    const tools = virtualServerToolsCache.get(name) || [];
    allServerInfos.push({
      name: virtualServer.name,
      status: 'connected',
      error: null,
      tools,
      client: createVirtualClient(virtualServer) as any,
  ...
  const filterServerInfos: ServerInfo[] = dataService.filterData
    ? dataService.filterData(allServerInfos)
    : allServerInfos;
...
export const registerVirtualServer = async (server: VirtualMcpServer): Promise<void> => {
  virtualServers.set(server.name, server);
  ...
  try {
    ...
    const tools = result.tools.map(t => ({
      ...
      inputSchema: cleanInputSchema(t.inputSchema || {}),
    }));
    virtualServerToolsCache.set(server.name, tools);
    console.log(`Registered virtual server: ${server.name} with ${tools.length} tools`);
  } catch (err) {
    console.error(`Failed to cache tools for virtual server ${server.name}:`, err);
  }
};

// Create a virtual client wrapper
const createVirtualClient = (server: VirtualMcpServer): VirtualClient => {
  return {
    listTools: () => server.listTools(),
    callTool: (toolParams: any, _meta?: any, _options?: any) => {
      // Handle both formats: direct (name, args) and structured ({name, arguments})
      if (typeof toolParams === 'string') {
        // Direct format: (name, args)
        return server.callTool(toolParams, _meta);
      } else {
        // Structured format: {name, arguments}
        return server.callTool(toolParams.name, toolParams.arguments || {});
      }
    },
    getServerCapabilities: () => server.getServerInfo?.() || {
      name: server.name,
      version: '1.0.0',
    ...
    close: async () => {
      // Virtual servers don't need closing
    },
  };
};

// Get server by name (including virtual servers)
const getServerByName = (name: string): ServerInfo | undefined => {
  // Check virtual servers first
  if (virtualServers.has(name)) {
    const virtualServer = virtualServers.get(name)!;
...
// Export getClient for external use
export const getClient = (name: string): any => {
...
const getServerByTool = (toolName: string): ServerInfo | undefined => {
  // First check virtual servers
  for (const [name, _virtualServer] of virtualServers) {
    const tools = virtualServerToolsCache.get(name) || [];
    ...
    if (request.params.name === 'call_tool') {
      ...
      if (extra && extra.server) {
        ...
        for (const [name, _virtualServer] of virtualServers) {
          const tools = virtualServerToolsCache.get(name) || [];
          if (tools.some((tool) => tool.name === toolName)) {
            targetServerInfo = getServerByName(name);
            break;
          }
        }
        
        // If not found in virtual servers, check regular servers
        if (!targetServerInfo) {
