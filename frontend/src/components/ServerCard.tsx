...
const ServerCard = ({ server, onRemove, onEdit, onToggle, onRefresh }: ServerCardProps) => {
  ...
  return (
    <>
      <div className={`bg-white shadow rounded-lg p-4 sm:p-6 mb-4 sm:mb-6 page-card transition-all duration-200 ${server.enabled === false ? 'opacity-60' : ''}`}>
        <div
          className="flex flex-col sm:flex-row sm:justify-between sm:items-center cursor-pointer space-y-3 sm:space-y-0"
          onClick={() => setIsExpanded(!isExpanded)}
        >
          <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
            <div className="flex items-center space-x-2">
              <h2 className={`text-lg sm:text-xl font-semibold ${server.enabled === false ? 'text-gray-600' : 'text-gray-900'}`}>{server.name}</h2>
              <StatusBadge status={server.status} />
            </div>

            <div className="flex items-center space-x-2">
              {/* Tool count display */}
              <div className="flex items-center px-2 py-1 bg-blue-50 text-blue-700 rounded-full text-xs sm:text-sm btn-primary">
              ...
              {server.error && (
                <div className="relative">
                  ...
                  {showErrorPopover && (
                    <div
                      ref={errorPopoverRef}
                      className="absolute z-50 mt-2 bg-white border border-gray-200 rounded-md shadow-lg p-0 left-0 sm:left-auto sm:right-0 w-[calc(100vw-2rem)] sm:w-96 max-w-lg"
                      style={{
                        maxHeight: '300px',
                        overflowY: 'auto'
                      }}
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <button
              onClick={handleEdit}
              className="px-2 py-1 sm:px-3 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 text-xs sm:text-sm btn-primary"
            >
              {t('server.edit')}
            </button>
            <ToggleSwitch
              enabled={server.enabled !== false}
              onToggle={() => handleToggle({} as React.MouseEvent)}
              disabled={isToggling}
              loading={isToggling}
            />
            <button
              onClick={handleRemove}
              className="px-2 py-1 sm:px-3 bg-red-100 text-red-800 rounded hover:bg-red-200 text-xs sm:text-sm btn-danger"
            >
              {t('server.delete')}
            </button>
            <button className="text-gray-400 hover:text-gray-600 btn-secondary p-1">
