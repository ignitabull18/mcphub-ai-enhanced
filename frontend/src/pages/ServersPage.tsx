  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 md:mb-8 space-y-4 sm:space-y-0">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{t('pages.servers.title')}</h1>
        <div className="flex flex-wrap gap-2 sm:gap-4">
          <button
            onClick={() => navigate('/market')}
            className="px-3 py-2 text-sm sm:px-4 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 flex items-center btn-primary transition-all duration-200"
          >
          ...
          >
            ...
            <span className="hidden sm:inline">{t('dxt.upload')}</span>
          </button>
          <button
            onClick={handleRefresh}
            disabled={isRefreshing}
            className={`px-3 py-2 text-sm sm:px-4 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 flex items-center btn-primary transition-all duration-200 ${isRefreshing ? 'opacity-70 cursor-not-allowed' : ''}`}
          >
            {isRefreshing ? (
